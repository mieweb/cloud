...
- **skopeo for image movement.** No Docker daemon needed to *distribute*: the CLI
  ...
  Replacing skopeo with a library is a deferred evaluation, not a plan item
  (see [Relationship to artipod](#relationship-to-artipod)).
...
mieweb ecosystem's layer for **durable pod state and execution attached to that
state**: a content-addressed OCI store, registry transports, and "realizers" that
run commands against a pod (bash isolate, hardened Docker/Podman). Because it
speaks OCI and drives Docker, the obvious question is whether this plan should
build on it. The ownership boundary settled in
[mieweb/artipod#56](https://github.com/mieweb/artipod/issues/56) says mostly no:

| Responsibility | Owner |
| --- | --- |
| Durable pod state, versioning/sync, executing commands against authorized pod state | artipod |
| Arbitrary long-lived HTTP application containers, Durable Object lifecycle, routing, deployment/scaling and production access policy | **this repo** (Cloud) |

Sharing OCI formats or dockerode plumbing is not enough reason to move
application-container lifecycle into artipod. Concretely for this plan:

- **Image distribution stays on skopeo** (M2 as written). Swapping to
  `@artipod/core/oci` is a separate, deferred evaluation — not a prerequisite or a
  commitment (#56 decision D6).
- **M4 is Cloud-owned.** The local/`mieweb` container adapter carries its own
  lifecycle (port proxy, `sleepAfter` idle timer, DO-id → container mapping) and
  pulls images with skopeo. A permissive "HTTP service" hardening profile inside
  artipod was rejected; its sandbox defaults are preserved.
- **What artipod *is* doing** (plan `cloud-container-56`, S0–S2, not started): a
  pod-scoped server-execution demo in `artipod serve` — browser bash gains an
  `ssh`-style adapter (`ssh server "find /"` → server isolate,
  `ssh container@server "uname -a"` → hardened Docker backend over authorized pod
  mounts). `serve` is a POC/reference host, not a production deployment path. Its
  S2 container backend is a useful *reference* for socket discovery and hardened
  `dockerode` use, not a dependency of M4.

### Why the boundary sits there — realizer semantics ≠ Container semantics

A *realizer* is artipod's term for the thing that turns a pod's declarative mount
table into something executable. Its Docker realizer answers "run a command
against versioned state, safely, offline". A Cloudflare `Container` answers "give
this Durable Object a sidecar HTTP service". The defaults are nearly inverse,
which is why forcing one to serve the other was declined:
...
Also relevant if the skopeo evaluation is ever reopened: artipod has no Dockerfile
builder (it imports trees / commits pods), its commits use a
`vnd.artipod.volume.v1+json` config rather than a runtime image config, and
`@artipod/core` carries ZenFS/just-bash/keyring/agent dependencies where
`@mieweb/cli` is zero-dependency shell-outs today.
...
- [ ] Lifecycle (port proxy, idle timer, DO-id → container mapping) is owned
      here, not delegated to artipod — see
      [Relationship to artipod](#relationship-to-artipod). artipod's S2 hardened
      Docker backend is a reference for socket discovery / `dockerode` use only.
